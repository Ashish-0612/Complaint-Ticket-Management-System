...
const WorkspacePage = ({ mode }) => {
  ...
  const config = {
    announcements: {
      title: "Announcements",
      subtitle: "Latest updates from the support team",
      icon: Bell,
    },
    activity: {
      title: "My Activity",
    ...
    reports: {
      title: "Reports",
      subtitle: "A quick overview of ticket performance",
      icon: BarChart3,
    },
    settings: {
      title: "Settings",
          ...
          {mode === "announcements" && (
            <div className="grid gap-4 md:grid-cols-3">
              {[
                [
                  "System Update",
                  "The complaint system is running normally. New tickets are reviewed by the support team.",
                  "Today",
                  "bg-blue-50",
                ],
                [
                  "Attachment Support",
                  "You can attach images and documents up to 5 MB to your complaint.",
                  "Recently",
                  "bg-green-50",
                ],
                [
                  "Resolution Reminder",
                  "Please add a comment if your resolved complaint still needs attention.",
                  "This week",
                  "bg-orange-50",
                ],
              ].map(([title, text, date, color]) => (
                <article
                  key={title}
              ...
              <div className="space-y-4">
                {tickets.slice(0, 10).map((ticket) => (
                  <button
                    ...
                    className="flex w-full cursor-pointer items-center gap-3 rounded-xl p-3 text-left transition hover:bg-gray-50"
                  >
            ...
            <section className="max-w-2xl rounded-2xl border border-gray-100 bg-white p-4 sm:p-5 shadow-sm">
              <div className="flex flex-wrap items-center justify-between gap-3 border-b border-gray-100 py-4">
                ...
                <button
                  type="button"
                  onClick={toggleTheme}
                  className="cursor-pointer rounded-xl bg-blue-600 px-4 py-2 text-sm font-semibold text-white"
                >
              ...
              <div className="flex items-center justify-between py-4">
                <div>
                  ...
                  <p className="mt-1 text-sm text-gray-500">
                    Notification preferences are available from the bell menu.
                  </p>
                </div>
